...
#
# STIP2021 scenarios are Baseline, STIP
# Specify the scenario when building
# Ex python build_network_mtc_stip2021.py net_spec_STIP2021.py Baseline
...
if __name__ == '__main__':
    ...
    parser.add_argument("netvariant", choices=["Baseline", "STIP"], help="Specify which network variant network to create.")
    ...
    # networks and log file will be in STIPNetworks
    if not os.path.exists("STIPNetworks"):
        os.mkdir("STIPNetworks")

    LOG_FILENAME = "stip%snetwork_%s_%s_%s.info.LOG" % ("TEST" if BUILD_MODE=="test" else "", PROJECT, NET_VARIANT, NOW)
    Wrangler.setupLogging(os.path.join("STIPNetworks",LOG_FILENAME),
                          os.path.join("STIPNetworks",LOG_FILENAME.replace("info", "debug")))
    ...
    for YEAR in NETWORK_PROJECTS.keys():
        ...
        hwypath=os.path.join("..", "STIPNetworks", "net_{}_{}".format(YEAR, NET_VARIANT), HWY_SUBDIR)
        if not os.path.exists(hwypath): os.makedirs(hwypath)
        trnpath = os.path.join("..", "STIPNetworks", "net_{}_{}".format(YEAR, NET_VARIANT), TRN_SUBDIR)
